...
import type { TeamStats, UserWithStats, StoryWithDetails, User } from "@/types";
...
export default function ManagerDashboard() {
  ...
  const filteredStories = useMemo(() => {
    ...
    return stories.filter((story) => {
      // Date filter
      if (filters.dateFrom && new Date(story.createdAt) < filters.dateFrom)
        return false;
      if (filters.dateTo && new Date(story.createdAt) > filters.dateTo)
        return false;
      ...
      // Status filter
      if (filters.status) {
        const score = story.coverageScore
          ? parseFloat(story.coverageScore)
          : null;
  ...
  const filteredUsers = useMemo(() => {
    const userList: UserWithStats[] = (users ?? []) as UserWithStats[];
    return userList.filter((user: UserWithStats) => {
      // User filter
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p
                  ...
                  data-testid="text-total-stories"
                >
          ...
          <CardContent className="pt-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p
                  className="text-2xl font-bold text-foreground"
                  data-testid="text-users-below-90"
                >
              ...
              {filteredUsers?.map((user) => (
                ...
                >
                  ...
                  <td
                    ...
                    data-testid={`text-story-count-${user.id}`}
                  >
                    {user.totalStories}
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      <span
                        ...
                        data-testid={`text-coverage-${user.id}`}
                      >
                      ...
                      <div className="ml-2 w-16 bg-muted rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${Math.min(user.averageCoverage, 100)}%`,
                          }}
                        ></div>
                      </div>
                    </div>
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border">
              {filteredStories?.slice(0, 10).map((story: StoryWithDetails) => (
                <tr
                  key={story.id}
                  className="hover:bg-accent"
                ...
                >
                  <td
                    className="px-6 py-4 font-mono text-sm text-primary"
                    data-testid={`text-ticket-${story.id}`}
                  >
                    {story.ticketId}
                  </td>
                  <td
                  ...
                  <td
                    className="px-6 py-4 text-foreground"
                    data-testid={`text-creator-${story.id}`}
                  >
                  ...
                  <td className="px-6 py-4">
                    {story.reviewer ? (
                      <span
                        ...
                        data-testid={`text-reviewer-${story.id}`}
                      >
                        {story.reviewer.firstName
                          ? `${story.reviewer.firstName} ${
                              story.reviewer.lastName || ""
                            }`
                      ...
                      >
                        <SelectTrigger
                          className="w-40"
                          data-testid={`select-reviewer-${story.id}`}
                        >
                    ...
                    {story.coverageScore ? (
                      editingCoverage[story.id] !== undefined ? (
                        ...
                        <div className="flex items-center space-x-2">
                          <span
                            className="text-foreground font-medium"
                            data-testid={`text-coverage-${story.id}`}
                          >
                  ...
                  <td
                    className="px-6 py-4"
                    data-testid={`text-status-${story.id}`}
                  >
